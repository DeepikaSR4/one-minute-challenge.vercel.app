...
export default async function MilestonePage({
    ...
    if (![10, 20, 30].includes(dayNumber)) {
        redirect('/dashboard')
    }
    ...
    // Fetch the milestone analytics from DB (mocked for demo)
    const stats = {
        avg_score: 41.5,
        improvement_percent: 28,
        filler_reduction: 45,
        strongest_skill: 'Clarity',
        weakest_skill: 'Conciseness',
    ...
    return (
        <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-2xl mx-auto w-full pb-20">
            ...
            {/* Shareable Card Area */}
            <div id="shareable-card" className="relative glass p-8 sm:p-12 rounded-3xl border-neon-blue/40 shadow-[0_0_50px_rgba(0,240,255,0.15)] overflow-hidden">
                {/* Glows */}
